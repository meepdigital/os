...
Usage: ./sh/persist [--device /dev/sdX] [--resume] [--minimal]
       ...
       ./sh/persist --iso [--source-iso /path/source.iso] [--root /meep] [--minimal]
       ./sh/persist --build-only [--source-iso /path/source.iso] [--root /meep] [--minimal]
...
persist_phase_00_preflight() {
  ...
  [[ ${MODE} == create && -z ${DEVICE} ]] || ((BUILD_ONLY == 0)) || {
    echo "--iso/--build-only cannot use --resume or a device." >&2
    exit 1
  }
}
