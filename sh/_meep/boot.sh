...
# Overlay build roots have no GRUB target disk. sh/persist owns their ISO menu.
...
[[ -x ${SCRIPT_DIR}/installer/node_modules/electron/dist/electron ]] || {
    echo "Missing local Electron runtime; run sh/_meep/npm.sh before boot integration" >&2
    exit 1
}
...
if [[ ${MEEP_LIVE_BUILD:-0} == 1 ]]; then
    ...
    echo "Meep graphics verified in the installed /boot initramfs images. A live USB requires a new ISO export via sh/persist."
fi
